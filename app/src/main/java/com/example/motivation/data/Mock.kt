...
class Mock {
    ...
    fun getPhrase(category: Int): String{
        val randomNumber = Random.nextInt(mListPhrase.filter { it.categoryId == category ||  category == all}.size)
        val filtered = mListPhrase.filter { it.categoryId == category ||  category == all }
